...
export function serializeGameStateForFirestore(gameState: GameState): FirestoreGameState {
  ...
  return {
    ...
    removedPieces: {
      Blue: gameState.removedPieces.Blue.map((piece) => ({ ...piece })),
      Red: gameState.removedPieces.Red.map((piece) => ({ ...piece })),
    },
    cannotActPieceIds: [...gameState.cannotActPieceIds],
...
export function deserializeGameStateFromFirestore(data: FirestoreGameState): GameState {
  ...
  return {
    ...
    removedPieces: {
      Blue: data.removedPieces?.Blue ?? [],
      Red: data.removedPieces?.Red ?? [],
    },
    cannotActPieceIds: data.cannotActPieceIds ?? [],
...
export function deserializePendingCombatFromFirestore(entry: FirestorePendingCombat): PendingCombat {
  return {
    ...
    smokeBombState: entry.smokeBombState ?? undefined,
    lastStrikeState: entry.lastStrikeState ?? undefined,
    boneSacrificeState: entry.boneSacrificeState ?? undefined,
